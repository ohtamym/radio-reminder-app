...
4. 最初のタスクを自動生成（tasksテーブルに挿入）
   ...
   - 毎週（weekly）の場合: **前回（1週間前）**の該当曜日・時刻のタスクを1件生成
...
- **毎週（weekly）の番組の場合は、上記で計算した日時から7日間引く**
  - 例1: 12/9(火)15:26に、水曜22時の番組を登録（毎週）
    - 次回放送: 12/11(水) 22:00 → **初回タスク: 12/3(水) 22:00**（前回）
    - 聴取期限: 12/11(木) 5:00
  - 例2: 12/9(火)15:26に、木曜27時の番組を登録（毎週）
    - 次回放送: 12/12(木) 27:00 → **初回タスク: 12/4(木) 27:00**（前回）
    - 聴取期限: 12/12(金) 5:00
